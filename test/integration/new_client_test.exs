defmodule KafkaEx.New.Client.Test do
  ...
  describe "list_offsets/1" do
    test "list offsets for empty topic", %{client: client} do
      topic_name = generate_random_string()
      _ = create_topic(client, topic_name)
      partition = [%{partition_num: 0, timestamp: -1}]

      {:ok, [offset]} = GenServer.call(client, {:list_offsets, [{topic_name, partition}], []})
    ...
    test "lists offsets for topic with existing messages", %{client: client} do
      topic_name = generate_random_string()
      _ = create_topic(client, topic_name)
