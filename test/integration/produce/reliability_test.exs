defmodule KafkaEx.Integration.Produce.ReliabilityTest do
  ...
  describe "produce across multiple topics on a single client" do
    test "each response echoes the topic it was sent to", %{client: client} do
      ...
      a_results =
        for i <- 1..5 do
          {:ok, %RecordMetadata{} = meta} = API.produce(client, topic_a, 0, [%{value: "a#{i}"}])
          meta
        end

      b_results =
        for i <- 1..5 do
          {:ok, %RecordMetadata{} = meta} = API.produce(client, topic_b, 0, [%{value: "b#{i}"}])
          meta
        end
    ...
    test "interleaved produce across two topics preserves topic identity per response",
         %{client: client} do
      topic_a = generate_random_string()
      topic_b = generate_random_string()
      _ = create_topic(client, topic_a)
      ...
      interleaved =
        for i <- 1..10 do
          ...
          {:ok, %RecordMetadata{} = meta} = API.produce(client, topic, 0, [%{value: "msg#{i}"}])
          {topic, meta.topic}
        end

      for {requested, returned} <- interleaved do
        assert requested == returned, "topic mismatch: requested #{requested}, got #{returned}"
      end
    end
  end
end
