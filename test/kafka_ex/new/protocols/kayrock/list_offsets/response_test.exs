defmodule KafkaEx.New.Protocols.Kayrock.ListOffsets.ResponseTest do
  ...
  describe "parse_response/1" do
    @expected_offset %KafkaEx.New.Structs.Offset{
      topic: "test-topic",
      partition: 1,
      offset: 0,
      timestamp: nil
    ...
    test "for api version 0 - returns error if any group failed" do
      ...
      expected_error = %KafkaEx.New.Structs.Error{
      ...
      response = %V1.Response{
        responses: [
          %{
            topic: "test-topic",
            partition_responses: [
              %{offset: 0, partition: 1, error_code: 1, timestamps: 1_000_000_000_000}
            ]
          }
        ],
        correlation_id: 3
    ...
    test "for api version 2 - returns response if all groups succeeded" do
      ...
      result_with_timestamp = Map.put(@expected_offset, :timestamp, 1_000_000_000_000)
