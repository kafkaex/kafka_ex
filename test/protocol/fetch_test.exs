defmodule Kafka.Protocol.Fetch.Test do
  ...
  test "parse_response correctly parses a valid response with a key and a value" do
    ...
    assert {:ok,
            %{"bar" => %{0 => %{:error_code => 0, :hw_mark_offset => 10, :message_set => [%{:attributes => 0, :crc => 0, :key => "foo", :value => "bar"}]}}},
            %{:client_id => "foo", :correlation_id => 1}} = Kafka.Protocol.Fetch.parse_response(%{:correlation_id => 1, :client_id => "foo"}, response)
  end
  ...
  test "parse_response correctly parses a valid response with a key and a nil value" do
    ...
    assert {:ok,
            %{"bar" => %{0 => %{:error_code => 0, :hw_mark_offset => 10, :message_set => [%{:attributes => 0, :crc => 0, :key => "foo", :value => nil}]}}},
            %{:client_id => "foo", :correlation_id => 1}} = Kafka.Protocol.Fetch.parse_response(%{:correlation_id => 1, :client_id => "foo"}, response)
  end
