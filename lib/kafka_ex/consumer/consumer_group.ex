defmodule KafkaEx.Consumer.ConsumerGroup do
  @moduledoc """
  ...
  `KafkaEx.Consumer.ConsumerGroup.PartitionAssignment` for details on partition
  ...
  * `:partition_assignment_callback` - See
     `t:KafkaEx.Consumer.ConsumerGroup.PartitionAssignment.callback/0`
  * `:uris` - List of Kafka broker URIs in `{"host", port}` format
  ...
  behaviour.
   - a tuple of `{gen_consumer_module, consumer_module}` can substitute another
     ...
     is passed it is transformed to `{KafkaEx.Consumer.GenConsumer, consumer_module}`.
  ...
  *Note* When starting a consumer group with multiple topics, you should
  ...
  def start_link(consumer_module, group_name, topics, opts)
    ...
    start_link({KafkaEx.Consumer.GenConsumer, consumer_module}, group_name, topics, opts)
  end
  ...
  @doc """
  Returns the pid of the `KafkaEx.Consumer.GenConsumer.Supervisor` that supervises this
  member's consumers.
  ...
  Returns an empty list if called before the initial sync.
  """
  @spec consumer_pids(Supervisor.supervisor()) :: [pid]
  def consumer_pids(supervisor_pid) do
    case consumer_supervisor_pid(supervisor_pid) do
      nil -> []
      pid -> GenConsumer.Supervisor.child_pids(pid)
    end
  end
  ...
  @doc """
  Returns the pid of the `KafkaEx.Consumer.ConsumerGroup.Manager` process for the
  given consumer group supervisor.

  Intended for introspection usage only.

  Raises `ArgumentError` if the manager process is not found or not running.
  """
  ...
  def get_manager_pid(supervisor_pid) do
    ...
    |> Enum.find_value(fn
      {KafkaEx.Consumer.ConsumerGroup.Manager, pid, _, _} -> pid
      _ -> nil
    end)
    |> case do
      nil -> raise ArgumentError, "Manager process not found in supervisor #{inspect(supervisor_pid)}"
      :restarting -> raise ArgumentError, "Manager process is restarting"
      :undefined -> raise ArgumentError, "Manager process is undefined"
      pid -> pid
    end
  end
  ...
  def start_consumer(
    ...
    child =
      Supervisor.child_spec(
        {
          KafkaEx.Consumer.GenConsumer.Supervisor,
          %{
