defmodule KafkaEx.Auth.SASL.CodecBinary do
  ...
  def api_versions_request(corr, ver, client_id \\ "kafka_ex") do
      <<
        @api_versions_key::16, ver::16, corr::32,
  ...
  def handshake_request(mech, corr, ver, client_id \\ "kafka_ex") do
      <<
        @sasl_handshake_key::16, ver::16, corr::32,
        byte_size(client_id)::16, client_id::binary,
  ...
  def authenticate_request(auth_bytes, corr, ver, client_id) do
      <<
        @sasl_authenticate_key::16, ver::16, corr::32,
        byte_size(client_id)::16, client_id::binary,
        byte_size(auth_bytes)::32, auth_bytes::binary
      >>
  end
  ...
  defp compact_bytes(<<>>), do: <<1>>  # Empty bytes = length 1
  ...
  defp read_compact_bytes(<<1, rest::binary>>), do: {<<>>, rest}  # Empty bytes
  ...
  defp skip_tags(n, bin) do
    {_, r1} = uvarint_decode(bin)  # tag_id
    {sz, r2} = uvarint_decode(r1)  # size
